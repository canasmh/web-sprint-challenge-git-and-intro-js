...
function removeArtist(artists, index) {
  const artistsCopy = artists.filter(artist => artist.id !== index);
...
function lotsOfArt(artists) {
  let artistsWithManyPaintings = [] //artists.filter(artist => artist.paintings > 100);

  for (let artist of artists) {
    if (artist.paintings > 100) {
      artistsWithManyPaintings.push(artist.name)
    }
  }

  return artistsWithManyPaintings;
}
...
function artistInfo(artists, name){
  ...
  return artists.filter(artist => artist.name === name)[0].bio;
...
function artistByCountry(artists, nationality){
  ...
  let namesOfNationality = [];
  for (let artist of artists) {
    if (artist.nationality === nationality) {
      namesOfNationality.push(artist.name);
    }
  }

  return namesOfNationality;
}
